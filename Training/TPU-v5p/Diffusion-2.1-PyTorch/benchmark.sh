#!/bin/bash
# SCP the environment setup to all instances. Used in `--env-file` in `docker run` on the
# host script
...
gcloud compute tpus tpu-vm scp --recurse  $TPU_NAME:~/profile/ /tmp/home/ --project $PROJECT --zone=$ZONE
